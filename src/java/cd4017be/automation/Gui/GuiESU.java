...
public class GuiESU extends GuiMachine
{
	...
	protected void drawGuiContainerForegroundLayer(int mx, int my) 
	{
		...
		double I = tileEntity.netF1 / (tileEntity.netF1 <= 0 ? -(float)tileEntity.netI0 : (float)Math.sqrt((double)tileEntity.netI0 * (double)tileEntity.netI0 + (double)tileEntity.netF1));
		this.drawFormatInfo(8, 46, 160, 4, "esu.energyFlow", tileEntity.netF1 / 1000F, I);
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) 
	{
		...
		this.drawStringCentered(tileEntity.netI0 + "V", this.guiLeft + 43, this.guiTop + 20, 0x404040);
		this.drawStringCentered((int)(tileEntity.netF0 / 1000F) + "/" + tileEntity.getMaxStorage() + " " + TooltipInfo.getEnergyUnit(), this.guiLeft + 88, this.guiTop + 56, 0x404040);
	...
	protected void mouseClicked(int x, int y, int b) throws IOException 
	{
		...
		if (a >= 0)
		{
			...
			if (tileEntity.netI0 > Config.Umax[tileEntity.type]) tileEntity.netI0 = Config.Umax[tileEntity.type];
			PacketBuffer dos = tileEntity.getPacketTargetData();
			dos.writeByte(AutomatedTile.CmdOffset);
			dos.writeInt(tileEntity.netI0);
