...
public class GuiSecuritySys extends GuiMachine {
	...
	public void initGui() {
		...
		guiComps.add(new ProgressBar(3, 137, 16, 16, 52, 179, 0, (byte)1));
		...
		guiComps.add(new Slider(7, 8, 151, 36, 179, 52, 8, 12, false));
		guiComps.add(new Button(8, 137, 145, 34, 12, 0).texture(179, 72).setTooltip("security.load"));
		guiComps.add(new Button(9, 137, 157, 34, 12, 0).texture(179, 72).setTooltip("security.admin"));
		guiComps.add(new Button(10, 137, 169, 34, 12, 0).texture(179, 72).setTooltip("security.special"));
		guiComps.add(new Button(11, 137, 181, 34, 12, 0).texture(179, 72).setTooltip("security.special"));
		for (int i = 0; i < 6; i++)
			guiComps.add(new Button(12 + i, 18, 145 + i * 8, 8, 8, 0).texture(187, 52));
		for (int i = 0; i < 6; i++)
			guiComps.add(new Text(18 + i, 27, 145 + i * 8, 108, 8, "\\%s"));
		guiComps.add(new Text(24, 137, 147, 34, 48, "security.lists").center().font(0x404040, 12));
		guiComps.add(new Matrix(25, 8, 16, 128, 128));
		guiComps.add(new Tooltip(26, 137, 16, 16, 52, "Estor1"));
	}
	...
	protected void setDisplVar(int id, Object obj, boolean send) {
		...
		case 7: scroll = Math.round((Float)obj * (float)(listS - 6)); return;
	...
	class Matrix extends GuiComp {
		...
		public void draw() {
			...
			drawTexturedModalRect(px + 60, py, 231, 0, 7, 7);
			drawTexturedModalRect(px + 60, py + 120, 231, 7, 7, 7);
			drawTexturedModalRect(px, py + 60, 231, 14, 7, 7);
			drawTexturedModalRect(px + 120, py + 60, 231, 21, 7, 7);
		}
