...
public class GuiAntimatterFabricator extends GuiMachine {
	...
	public void initGui() {
		...
		this.ySize = 168;
		super.initGui();
		guiComps.add(new NumberSel(5, 53, 16, 70, 16, "%dV", 0, Config.Umax[2], 10).setup(8, 0xff404040, 2, true).setTooltip("voltage"));
		guiComps.add(new Button(6, 52, 51, 18, 18, 0).texture(184, 16).setTooltip("rstCtr"));
		guiComps.add(new ProgressBar(7, 53, 38, 70, 8, 184, 0, (byte)0));
		guiComps.add(new Tooltip(8, 53, 38, 70, 8, "progress"));
		guiComps.add(new Text(9, 70, 56, 54, 8, "antimFab.power").center());
	}
	...
	protected Object getDisplVar(int id) {
		...
		case 9: return String.format("%." + (tile.power >= 100 ? "0" : "1") + "f", tile.power);
		default: return null;
		}
	}
	...
	protected void setDisplVar(int id, Object obj, boolean send) {
		...
		switch(id) {
		case 5: dos.writeByte(AutomatedTile.CmdOffset).writeInt(tile.Uref = (Integer)obj); break;
		case 6: dos.writeByte(AutomatedTile.CmdOffset + 1).writeByte(tile.rs = (tile.rs + ((Integer)obj == 0 ? 1 : 2)) % 4); break;
		}
