...
public class GuiVoltageTransformer extends GuiMachine
{
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) 
    {
        ...
        if ((tileEntity.netData.ints[1] & 1) == 0)
        {
            this.drawTexturedModalRect(this.guiLeft + 97, this.guiTop + 15, 176, 18, 36, 18);
            this.drawStringCentered((tileEntity.netData.ints[1] & 2) == 0 ? "Off" : "On", this.guiLeft + 151, this.guiTop + 20, 0x404040);
        } else
        {
            this.drawTexturedModalRect(this.guiLeft + 97, this.guiTop + 15, 176, 0, 36, 18);
            this.drawTexturedModalRect(this.guiLeft + 133, this.guiTop + 15, 212, (tileEntity.netData.ints[1] & 2) == 0 ? 0 : 18, 36, 18);
        }
        this.drawStringCentered(String.format("%.1f x", (float)tileEntity.netData.ints[0] * 0.1F), this.guiLeft + 43, this.guiTop + 20, 0x404040);
    ...
    protected void mouseClicked(int x, int y, int b) throws IOException 
    {
        ...
        if (d)
        {
            if (tileEntity.netData.ints[0] < 10) tileEntity.netData.ints[0] = 10;
            if (tileEntity.netData.ints[0] > 200) tileEntity.netData.ints[0] = 200;
            PacketBuffer dos = tileEntity.getPacketTargetData();
            dos.writeInt(tileEntity.netData.ints[0]);
            dos.writeByte(tileEntity.netData.ints[1]);
