...
public class GuiTeleporter extends GuiMachine
{
	...
	public void updateScreen() 
	{
		...
		if (!tfX.isFocused()) tfX.setText("" + tileEntity.netI0);
		if (!tfY.isFocused()) tfY.setText("" + tileEntity.netI1);
		if (!tfZ.isFocused()) tfZ.setText("" + tileEntity.netI2);
	}
	...
	protected void drawGuiContainerForegroundLayer(int mx, int my) 
	{
		...
		this.drawFormatInfo(8, 34, 88, 16, "storage", (long)Math.floor(tileEntity.netF0 / 1000F), (long)Math.floor(tileEntity.netF1 / 1000F));
		this.drawInfo(134, 52, 16, 16, "\\i", "teleport.rw");
		this.drawInfo(98, 34, 16, 16, "\\i", "rstCtr");
		this.drawInfo(116, 34, 16, 16, "\\i", "teleport." + ((tileEntity.netI3 & 2) == 0 ? "abs" : "rel"));
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) 
	{
		...
		if ((tileEntity.netI3 & 1) == 0) this.drawTexturedModalRect(this.guiLeft + 98, this.guiTop + 34, 176, 0, 16, 16);
		this.drawStringCentered((tileEntity.netI3 & 2) == 0 ? "abs" : "rel", this.guiLeft + 124, this.guiTop + 38, 0x404040);
		this.drawLocString(this.guiLeft + 9, this.guiTop + 38, 0, 0x404040, "teleport." + ((tileEntity.netI3 & 4) == 0 ? "teleport" : "charge"));
		tfX.drawTextBox();
		tfY.drawTextBox();
		tfZ.drawTextBox();
		name.drawTextBox();
		...
		this.drawStringCentered(I18n.translateToLocal("gui.cd4017be.teleport." + ((tileEntity.netI3 & 16) != 0 ?"copy":"move")), this.guiLeft + this.xSize * 3 / 4, this.guiTop + 4, 0xff4040);
		showWarning();
	...
	protected void mouseClicked(int x, int y, int b) throws IOException 
	{
		...
		if (this.isPointInRegion(7, 33, 90, 18, x, y)) {
			if (warned == 1 && (tileEntity.netI3 & 4) == 0) warned = 2;
			else kb = 0;
		} else
