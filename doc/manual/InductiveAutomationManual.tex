...
\imgtex
\includegraphics[align = t]{blockCompressor} & One of the first machines to build is a \bf compression assembler \rm as it's needed to craft most parts required for other machines. Its recipes are shaped so every item must be placed into the correct slot. There are two tiers of this machine: The \bf steam pneumatic compressor \rm is powered by steam and requires 1 second per operation consuming 800L steam (used at the beginning in combination with a steam boiler). And the \bf electromagnetic compressor \rm can do up to one operation per tick using 200KJ electric energy.\\ \hline
